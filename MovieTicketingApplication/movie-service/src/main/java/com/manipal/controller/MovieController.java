...
public class MovieController {
	...
	@PutMapping("movie-service/movie/{movieId}")
	public ResponseEntity<Movie> updateMovie(@PathVariable("movieId") int movieId, @RequestBody Movie movieDetails) {
		Movie movie = movieService.getMovieById(movieId);
		
		movie.setMovieName(movieDetails.getMovieName());
		movie.setRating(movieDetails.getRating());
		movie.setTimeSlot(movieDetails.getTimeSlot());
		movie.setAvailableNoOfTickets(movieDetails.getAvailableNoOfTickets());
		
		Movie updatedMovie =  movieService.setMovie(movie);
